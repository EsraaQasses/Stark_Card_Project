...
from .models import (
    Section, Product, ProductRequirement, Package, 
    PackagePrice, Favorite, ExternalProduct, StoreProduct
)
...
class ProductAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'currency', 'base_price', 'is_active', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        ('Pricing', {
            'fields': ('product_type', 'currency', 'base_price')
        }),
        ('Amount Based Settings', {
            'fields': ('min_amount', 'max_amount', 'min_amount_price', 'price_per_unit'),
            'classes': ('collapse',)
        }),
...
class PackageAdmin(admin.ModelAdmin):
    ...
    search_fields = ['name', 'product__name_en']
    autocomplete_fields = ['product']
    list_per_page = 20

@admin.register(PackagePrice)
class PackagePriceAdmin(admin.ModelAdmin):
    list_display = ['package', 'currency', 'amount']
