...
class ProductSyncRun(models.Model):
    STATUS_CHOICES = [
        ("running", "Running"),
        ("succeeded", "Succeeded"),
        ("partial", "Partial"),
    ...
    api_config = models.ForeignKey(ThirdPartyAPI, on_delete=models.CASCADE, related_name="sync_runs")
    ...
    class Meta:
        ordering = ["-started_at"]
        indexes = [models.Index(fields=["provider", "started_at"]), models.Index(fields=["status", "started_at"])]


class ProductSyncResult(models.Model):
    STATUS_CHOICES = [
        ("active", "Active"),
        ("inactive", "Inactive"),
        ("removed", "Removed"),
    ...
    run = models.ForeignKey(ProductSyncRun, on_delete=models.CASCADE, related_name="results")
    external_product = models.ForeignKey(
        "store.ExternalProduct", on_delete=models.SET_NULL, null=True, blank=True, related_name="sync_results"
    )
    ...
    class Meta:
        indexes = [models.Index(fields=["run", "status"]), models.Index(fields=["external_id"])]
